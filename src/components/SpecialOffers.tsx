      ...
      <div className="container mx-auto">
        <div className="text-center mb-12 animate-slide-up">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            ⭐ {t('sections.specialOffersSubtitle')} ✨
          </p>
          <div className="mt-4 flex items-center justify-center gap-4 text-sm font-medium text-primary">
            <span className="flex items-center gap-1">🔥 {t('sections.hotDeals')}</span>
            <span>•</span>
            <span className="flex items-center gap-1">⏰ {t('sections.limitedTime')}</span>
            <span>•</span>
            <span className="flex items-center gap-1">🏆 {t('sections.bestPrices')}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {specialOffers.map((offer, index) => {
            ...
            return (
              <Card key={offer.id} className="group overflow-hidden border-0 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-[1.02] bg-card animate-fade-in" style={{ animationDelay: `${index * 100}ms` }}>
                <div className="relative overflow-hidden">
                  <img src={offer.image} alt={offer.title} className="w-full h-64 object-cover group-hover:scale-125 group-hover:rotate-2 transition-all duration-700" loading="lazy" decoding="async" />
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star key={i} className={`w-3 h-3 transition-all duration-300 ${i < Math.floor(offer.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-400'}`} />
                        ))}
                        <span className="text-xs ml-1 font-bold">({offer.rating})</span>
                      </div>
                      <span className="text-xs opacity-75">• {offer.reviews} {t('common.reviews')}</span>
                    </div>
                  </div>
                </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-center">
                    {offer.inclusions ? (
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-3 gap-4 text-center">
                              ...
                              <div className="space-y-1">
                                ...
                                <p className="text-sm font-semibold">{offer.rating} ({offer.reviews} {t('common.reviews')})</p>
                              </div>
                            </div>
                            ...
                            <Tabs defaultValue="overview" className="w-full">
                              <TabsList className="grid w-full grid-cols-3">
                                ...
                                <TabsTrigger value="inclusions">{t('package.whatsIncluded')}</TabsTrigger>
                                <TabsTrigger value="itinerary">{t('package.itinerary')}</TabsTrigger>
                              </TabsList>

                              <TabsContent value="overview" className="space-y-4">
                                <div>
                                  <h3 className="font-semibold text-lg mb-2">{t('package.aboutPackage')}</h3>
                                  <p className="text-muted-foreground">{offer.detailedDescription}</p>
                                </div>
                                <div>
                                  <h3 className="font-semibold text-lg mb-3">{t('package.packageHighlights')}</h3>
                              ...
                              <TabsContent value="inclusions" className="space-y-3">
                                <h3 className="font-semibold text-lg mb-3">{t('package.whatsIncluded')}</h3>
                              ...
                              <TabsContent value="itinerary" className="space-y-4">
                                <h3 className="font-semibold text-lg mb-3">{t('package.activityProgram')}</h3>
                            ...
                            <div className="border-t pt-6 flex items-center justify-between">
                              <div>
                                ...
                                <span className="text-sm text-muted-foreground">{t('common.perPerson')} • {offer.duration}</span>
                              </div>
                              <BookingDialog
                      ...
                      <BookingDialog packageName={offer.subtitle} packagePrice={offer.price} buttonText={t('common.bookNow')} buttonVariant="default" buttonSize="lg" />
                    )}
                  </div>
                </CardContent>
              </Card>
        ...
        <div className="text-center mt-12">
          <p className="text-sm text-muted-foreground mt-4">💰 {t('sections.saveUpTo')}</p>
        </div>
      </div>
    </section>
  );
